...
import { ProductLocationModel } from './product-location.model.js';
...
declare var $: any;
@Component({
    selector: 'product-location-grid',
    providers: [ProductLocationService],
    templateUrl: './product-location.html'
})

export class ProductLocation extends Property {
    ...
    CurrentProductLocationObj: ProductLocationModel = new ProductLocationModel();
    constructor(
        private _util: Util, private _menu: SidebarService, private _router: Router,
        private productLocationService: ProductLocationService, public commonService: CommonService, 
    ...
    setDataSource() {
        ...
        this.dataSource = {
            ...
            getRows: (params: any) => {
                ...
                this.productLocationService.loadAll(params.startRow, params.endRow, sortColID, sortDirection, this.filterText, this.partnerID).
                    subscribe(
                        result => {
                            debugger;
                            ...
                            this.IsLoaded = true;                            
                            //debugger;
                            var coldef = this.h_localize.find(element => element.field == "LocationCode");
    ...
    onSelectionChanged() {
        ...
        this.CurrentProductLocationObj = this.gridapi.getSelectedRows()[0];
        if (!this.CurrentProductLocationObj) {
            this.isEditConfigSetup$ = false;
            this.isDeleteConfigSetup$ = false;
        }
    }
    ...
    onRowClicked(e) {
        if (e.event.target !== undefined) {
            ...
            if (actionType == "select") {
                ...
                this.CurrentProductLocationObj = data;
    ...
    EditClicked(val) {
        this.loaderService.display(true);
        this.CurrentProductLocationObj = val;
        this.onEditProductLocation();
    }
    ...
    onAddProductLocation() {
        this.ListView = false;
        //this.IsLoaded = true;
        this.CurrentProductLocationObj = new ProductLocationModel();
        if (this.CurrentProductLocationObj.IsActive == undefined) {
            this.CurrentProductLocationObj.IsActive = true;
        }        
    }

    onEditProductLocation() {
        debugger;
        this.ListView = false;

        if (this.CurrentProductLocationObj.IsActive.toString() == 'Yes') {
            this.CurrentProductLocationObj.IsActive = true;
        }
        else {
            this.CurrentProductLocationObj.IsActive = false;
        }   
        this.loaderService.display(false);     
    }
    ...
    Save(form) {
        ...
        this.CurrentProductLocationObj.UserID = this.UserID
        if (this.CurrentProductLocationObj.ID == undefined) {
            this.CurrentProductLocationObj.ID = 0;
        }

        //  console.log(JSON.stringify(this.CurrentProductLocationObj));
        this.productLocationService.Save(this.CurrentProductLocationObj)
