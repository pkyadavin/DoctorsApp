...
export class _returnsObject {
  ...
  constructor(_returns?: any) {
    this.created_date = (_returns && _returns.created_date) || "";
...
export class _returns extends _returnsObject {
  constructor(private _m: ReturnsService) {
    super();
  }
  getpassport() {}
  getOrder() {}
...
export class items {
  ...
  constructor(_items?: any) {
    ...
    this.itemId = (_items && _items.itemId) || "";
    this.extra =
    ...
    this.item_status =
      (_items &&
        _items.item_status &&
        $.map(TypedJson.parse<event>(_items.item_status), function(v, i) {
          return new event(v);
        })) ||
      [];
...
export class event {
  ...
  constructor(_event?: any) {
    this.event = (_event && _event.event) || "";
    this.remark = (_event && _event.remark) || "";
    this.event_time = (_event && _event.event_time) || "";
    this.status =
      (_event && _event.status && new status(_event.status)) || new status();
  }
}
...
export class customer_display extends MaskString {
  ...
  constructor(_customer?: any) {
    super();
    this.cif_number = (_customer && _customer.cif_number) || "";
    this.name =
      (_customer &&
    ...
    this.address =
      _customer == undefined
        ? new address_display()
        : new address_display(JSON.parse(JSON.stringify(_customer.address)));
  }
}
