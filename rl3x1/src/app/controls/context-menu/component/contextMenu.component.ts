...
import { CONTEXT_MENU_OPTIONS, IContextMenuOptions } from './src/contextMenu.options.js';
import {
    AfterContentInit,
    ChangeDetectorRef,
    Component,
...
import { ContextMenuService,IContextMenuClickEvent } from './src/contextMenu.service.js';
import { ContextMenuItemDirective } from './src/contextMenu.item.directive.js';
...
@Component({
    ...
    styles: [`.angular2-contextmenu .dropdown-menu {
    ...
    `<div class="dropdown angular2-contextmenu">
      <ul #menu [ngStyle]="locationCss" class="dropdown-menu">
        ...
        <li *ngFor="let link of links" >
          <a href [class.dropdown-item]="useBootstrap4" 
...
export class ContextMenuComponent implements AfterContentInit {    
    ...
    public onMenuEvent(menuEvent: IContextMenuClickEvent): void {
        ...
        var leftvalue = event.clientX;
        var rightvalue = event.clientY;

        if (location.href.indexOf('ModelMaster') > 0) {
            leftvalue = (event.view.document.body.clientWidth) * 13 / 100;
            rightvalue = event.clientY - 80;
        }
