...
export class AlphabetOnlyDirective {
  ...
  private alphabets="abcdefghijklmnopqrstuvwzyz ";
  ...
  onKeyDown(e: KeyboardEvent) {
    ...
    if ( this.IsNotAlphabet(e.key)) {
      e.preventDefault();
    }
  }

  IsNotAlphabet(key){
    return !(this.alphabets.includes(key) || this.alphabets.toUpperCase().includes(key)) ;
  }
  ...
  private sanatizeInput(input: string): string {
    for (var char of input) {
       if (this.IsNotAlphabet(char)) return '';
    }
    return input;
  }
