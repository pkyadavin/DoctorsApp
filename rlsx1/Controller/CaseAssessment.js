...
router.get('/:StartIndex/:PageSize/:SortColumn/:SortDirection/:FilterValue/:PartnerID', function (req, res) {
    ...
    connection.connect().then(function () {
        ...
        request.execute('usp_GetAllCaseAssessment', function (err, recordsets, returnValue) {
            if (err) {
                Error.ErrorLog(err, "usp_GetAllCaseAssessment", JSON.stringify(request.parameters), req);
...
router.post('/', function (req, res) {
    ...
    connection.connect().then(function () {
        ...
        request.execute('usp_SaveCaseAssessment', function (err, recordsets) {
            if (err) {
                res.writeHead(200, { "Content-Type": "application/json" });
                res.write(JSON.stringify({ data: "Error Occured: " + err }));
            }
