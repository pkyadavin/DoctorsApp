...
router.get('/Request/:ID', function (req, res) {
    ...
    connection.connect().then(function () {
        var request = new sql.Request(connection);
        request.input('SRNumber', sql.NVarChar(sql.MAX), req.params.ID);
        request.input('LanguageID', sql.Int, req.userlogininfo.languageid);
        request.execute('usp_ServiceRequestOrder_Get', function (err, recordsets, returnValue) {
            if (err) {
                Error.ErrorLog(err, "usp_ServiceRequestOrder_Get", JSON.stringify(request.parameters), req);
                res.send({ error: err });
            }
            else {
...
router.get('/InitRepairOrderLine', function (req, res) {
    var connection = new sql.Connection(settings.DBconfig(req));
    connection.connect().then(function () {
        var request = new sql.Request(connection);        
        request.input('UserID', sql.Int, req.userlogininfo.UserID);
        request.input('LanguageID', sql.Int, req.userlogininfo.languageid);
        request.execute('usp_InitRepairOrderLine', function (err, recordsets, returnValue) {
            if (err) {
                Error.ErrorLog(err, "usp_InitRepairOrderLine", JSON.stringify(request.parameters), req);
                res.send({ error: err });
            }
            else {
...
router.get('/discard/:ID/:Reason', function (req, res) {
    var connection = new sql.Connection(settings.DBconfig(req));
    connection.connect().then(function () {
        var request = new sql.Request(connection);
        request.input('ID', sql.Int, req.params.ID);
        request.input('Reason', sql.NVarChar(sql.MAX), req.params.Reason);
        request.input('UserID', sql.Int, req.userlogininfo.UserID);
        request.input('LanguageID', sql.Int, req.userlogininfo.languageid);
        request.execute('usp_RmaOrder_Discard', function (err, recordsets, returnValue) {
            if (err) {
                Error.ErrorLog(err, "usp_RmaOrder_Discard", JSON.stringify(request.parameters), req);
                res.send({ error: err });
            }
            else {
                res.send({ recordsets: recordsets });
...
router.post('/', function (req, res) {
    ...
    connection.connect().then(function () {
        ...
        switch (req.body[0].orderType) {
            case "NEW":
                actionSP = "usp_RmaOrder_Post";
                break;
            case "REPAIR":
                actionSP = "usp_Repair_Post";
                break;
        }
        request.execute(actionSP, function (err, recordsets, returnValue) {
            if (err) {
