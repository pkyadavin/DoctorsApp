...
router.get('/:StartIndex/:PageSize/:SortColumn/:SortDirection/:FilterValue/:PartnerID', function (req, res) {
    ...
    connection.connect().then(function () {        
        ...
        request.execute('usp_GetRepairMaterials', function (err, recordsets, returnValue) {
            if (err) {
                Error.ErrorLog(err, "usp_GetRepairMaterials", JSON.stringify(request.parameters), req);
...
router.post('/', function (req, res) {
    ...
    connection.connect().then(function () {
        ...
        request.execute('usp_SaveReturnMaterial', function (err, recordsets) {
            if (err) {
                res.writeHead(200, { "Content-Type": "application/json" });
                res.write(JSON.stringify({ data: "Error Occured: " + err }));
            }
