...
router.get('/:StartIndex/:PageSize/:SortColumn/:SortDirection/:FilterValue/:PartnerID', function (req, res) {
    ...
    connection.connect().then(function () {        
        ...
        request.execute('usp_GetAllReturnReasonRootCause', function (err, recordsets, returnValue) {
            if (err) {
                Error.ErrorLog(err, "usp_GetAllReturnReasonRootCause", JSON.stringify(request.parameters), req);
...
router.post('/', function (req, res) {
    ...
    connection.connect().then(function () {
        ...
        request.execute('usp_SaveReturnReasonRootCause', function (err, recordsets) {
            if (err) {
                res.writeHead(200, { "Content-Type": "application/json" });
                res.write(JSON.stringify({ data: "Error Occured: " + err }));
            }
