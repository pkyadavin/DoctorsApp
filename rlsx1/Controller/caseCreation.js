...
router.post('/', function (req, res) {
    ...
    connection.connect(function (error) {
        ...
        request.execute('usp_Save_CaseCreation', function (err, recordsets, returnValue) {//old proc usp_SaveCaseCreation
            if (err) {
                res.writeHead(200, { "Content-Type": "application/json" });
                res.write(JSON.stringify({ data: "Error Occured: " + err }));
            }
...
router.get('/getLocation/:catcode', function(req,res){
    var catcode = req.params.catcode;
    var connection = new sql.Connection(settings.DBconfig(req));
    connection.connect(function (error) {
        var request = new sql.Request(connection);
        request.input("CategoryCode", sql.VarChar(50), catcode);
        request.execute('usp_getLocationFromCategory', function (err, recordsets, returnValue) {
            if (err) {
                Error.ErrorLog(err, 'usp_getLocationFromCategory', JSON.stringify(request.parameters), req);
                res.writeHead(200, { "Content-Type": "application/json" });
                res.write(JSON.stringify({ data: "Error Occured: " + err }));
            }
            else {
