...
router.get('/:StartIndex/:PageSize/:SortColumn/:SortDirection/:FilterValue/:PartnerID', function (req, res) {
    ...
    connection.connect().then(function () {        
        ...
        request.execute('usp_GetAllColor', function (err, recordsets, returnValue) {
            if (err) {
                Error.ErrorLog(err, "usp_GetAllColor", JSON.stringify(request.parameters), req);
...
router.get('/GetColorData/:ColorID', function (req, res) {
    var colorID = req.params.ColorID;
    var connection = new sql.Connection(settings.DBconfig(req));
    connection.connect().then(function () {
        var request = new sql.Request(connection);
        request.input('ColorID', sql.Int, colorID);
        request.execute('GetColorByColorID', function (err, recordsets, returnValue) {
            if (err) {
                Error.ErrorLog(err, 'GetColorByColorID', JSON.stringify(request.parameters), req);
            }
            res.send(recordsets);
...
router.post('/', function (req, res) {
    ...
    connection.connect().then(function () {
        ...
        request.execute('usp_SaveColor', function (err, recordsets) {
            if (err) {
                res.writeHead(200, { "Content-Type": "application/json" });
                res.write(JSON.stringify({ data: "Error Occured: " + err }));
            }
...
//Update
router.put('/:ColorID', function (req, res) {
    var colorId = req.params.ColorID;
    var connection = new sql.Connection(settings.DBconfig(req));
    connection.connect().then(function () {
        var request = new sql.Request(connection);
        ...
        request.output('result', sql.VarChar(5000));
        request.execute('usp_UpdateColorByColorID', function (err, recordsets) {
            if (err) {
                Error.ErrorLog(err, 'usp_UpdateColorByColorID', JSON.stringify(request.parameters), req);
            }
