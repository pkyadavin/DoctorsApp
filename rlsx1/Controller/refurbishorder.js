...
router.get('/:PartnerID/:OrderStatus/:StartIndex/:PageSize/:SortColumn/:SortDirection/:FilterValue/:OrderDayWise', function (req, res) {
    ...
    connection.connect().then(function () {
        ...
        request.execute('usp_RefurbishOrder_All', function (err, recordsets, returnValue) {
            if (err) {
                Error.ErrorLog(err, "usp_RefurbishOrder_All", JSON.stringify(request.parameters), req);
                res.send({ error: err });
            }
            else {
...
router.get('/:ID', function (req, res) {
    ...
    connection.connect().then(function () {
        ...
        request.input('LanguageID', sql.Int, req.userlogininfo.languageid);
        request.execute('usp_RefurbishOrder', function (err, recordsets, returnValue) {
            if (err) {
                Error.ErrorLog(err, "usp_RefurbishOrder", JSON.stringify(request.parameters), req);
                res.send({ error: err });
            }
            else {
...
router.get('/testlogs/:ID', function (req, res) {
    db.executeSql(req, `SELECT (SELECT DISTINCT Cast(rt.CreatedDate as Date)  AS [group],
            Case when Sum(Case RT.TestResult when 'Fail' then 1 else 0 end) > 0 then 'FAIL' ELSE 'PASS' END AS TestResult, rt.TestNumber,
             ...
             FROM RefurbRepairTestResult r INNER JOIN RMAActionCode c ON c.RMAActionCodeID = r.RMAActionCodeID WHERE r.TestNumber = rt.TestNumber
             FOR JSON PATH) as TestDetail
             FROM RefurbRepairTestResult rt INNER JOIN RMAActionCode ac ON ac.RMAActionCodeID = rt.RMAActionCodeID
             WHERE rt.RefurbRepairHistoryID = ${ req.params.ID} GROUP BY rt.TestNumber, Cast(rt.CreatedDate as Date) ORDER BY rt.TestNumber DESC for json path) as recordset`, function (data, err) {
            if (err) {
...
router.get('/item/:SerialNumber', function (req, res) {
    var connection = new sql.Connection(settings.DBconfig(req));
    connection.connect().then(function () {
        var request = new sql.Request(connection);
        var actionSP = "usp_RefurbDevice";
        request.input('SerialNumber', sql.NVarChar(sql.MAX), req.params.SerialNumber);
        request.execute(actionSP, function (err, recordsets, returnValue) {
            if (err) {
...
router.get('/repairparts/:ID/:pID', function (req, res) {
    db.executeSql(req, `SELECT (SELECT IP.PartnerID,
               ...
               FROM RefurbOrderDetail RMA 
                ...
                LEFT JOIN vw_ItemSerialInventoryLocationWise inv ON inv.ItemMasterID = I.ItemMasterID AND inv.PartnerID = p.PartnerID 
               ...
               where rma.RefurbOrderDetailID = ${req.params.ID} AND p.PartnerID = ${req.params.pID} for json path) AS parts`, function (data, err) {
            if (err) {
...
router.post('/', function (req, res) {
    ...
    connection.connect().then(function () {
        ...
        request.input('UserID', sql.Int, req.userlogininfo.UserID);
        request.input('LanguageID', sql.Int, req.userlogininfo.languageid);
        switch (req.body[0].orderType) {
            case "NEW":
                actionSP = "usp_RefurbOrder_Post";
                break;
            case "REPAIR":
                actionSP = "usp_RefurbRepair_Post";
                break;
        }
        request.execute(actionSP, function (err, recordsets, returnValue) {
            if (err) {
...
router.post('/repairaction', function (req, res) {
    ...
    connection.connect().then(function () {
        var request = new sql.Request(connection);
        var actionSP = "usp_Refurb_Allocate";
...
router.get('/quotation/:RMAOrderDetailID', function (req, res) {
    ...
    connection.connect().then(function () {
        var request = new sql.Request(connection);
        var actionSP = "usp_RefurbCostCalculator";
        request.input('RefurbOrderDetailID', sql.Int, req.params.RMAOrderDetailID);
        request.input('UserID', sql.Int, req.userlogininfo.UserID);
        request.execute(actionSP, function (err, recordsets, returnValue) {
            if (err) {
                Error.ErrorLog(err, actionSP, JSON.stringify(request.parameters), req);
...
router.get('/raisequotation/:RMAOrderDetailID/:PreApproved/:Amount', function (req, res) {
    ...
    connection.connect().then(function () {
        var request = new sql.Request(connection);
        var actionSP = "usp_RfbRaiseQuote";
