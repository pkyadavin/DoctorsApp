...
router.get('/:StartIndex/:PageSize/:SortColumn/:SortDirection/:FilterValue/:PartnerID', function (req, res) {
    ...
    connection.connect().then(function () {
        ...
        request.execute('GetLoadOrderByPaging', function (err, recordsets, returnValue) {
            if (err) {
                Error.ErrorLog(err, "GetLoadOrderByPaging", JSON.stringify(request.parameters), req);
            }
...
router.get('/GetLoadOrderById/:Id', function (req, res) {
    var connection = new sql.Connection(settings.DBconfig(req));
    connection.connect().then( function (error) {
       var request = new sql.Request(connection);
       request.input('Id', sql.Int, req.params.Id);
       request.execute('GetLoadOrderById', function (err, recordsets, returnValue) {
            if (err) {
                Error.ErrorLog(err, "GetLoadOrderById", JSON.stringify(request.parameters), req);
            }
            res.send(recordsets);
        })
    });
...
router.post('/', function (req, res) {
   ...
   connection.connect().then(function (error) {
       ...
       request.input('LoadOrderHeaderId', sql.Int, req.body.LoadOrderHeaderID);
       request.input('UserId', sql.Int, req.body.UserId);
       request.input('PartnerId', sql.Int, req.body.PartnerID);
       request.execute('CreateOrUpdateLoadOrder', function (err, recordsets, returnValue) {
           if (err) {
               Error.ErrorLog(err, "CreateOrUpdateLoadOrder", JSON.stringify(request.parameters), req);
           }
            res.send(recordsets);
        })
    });
