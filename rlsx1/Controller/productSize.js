...
router.get('/:StartIndex/:PageSize/:SortColumn/:SortDirection/:FilterValue/:PartnerID', function (req, res) {
    ...
    connection.connect(function (error) {
        ...
        request.execute('usp_getProductSize', function (err, recordsets, returnValue) {
            if (err) {
                Error.ErrorLog(err, 'usp_getProductSize', JSON.stringify(request.parameters), req);
...
router.post('/', function (req, res) {
    ...
    connection.connect(function (error) {
        ...
        request.execute('usp_SaveProductSize', function (err, recordsets, returnValue) {
            if (err) {
                res.writeHead(200, { "Content-Type": "application/json" });
                res.write(JSON.stringify({ data: "Error Occured: " + err }));
            }
...
router.delete('/Remove/:ID', function (req, res) {
    ...
    db.executeSql(req, 'delete from ProductSize where ProductSizeID =' + id, function (err, data) {
        if (err) {
            res.writeHead(200, { "Content-Type": "application/json" });
            res.write(JSON.stringify({ data: "Error Occured: " + err }));
        }
