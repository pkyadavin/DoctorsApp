...
router.get('/:StartIndex/:PageSize/:SortColumn/:SortDirection/:FilterValue/:PartnerID', function (req, res) {
    ...
    connection.connect(function (error) {
        ...
        request.execute('usp_getProductGrade', function (err, recordsets, returnValue) {
            if (err) {
                Error.ErrorLog(err, 'usp_getProductGrade', JSON.stringify(request.parameters), req);
...
router.post('/', function (req, res) {
    ...
    connection.connect(function (error) {
        ...
        request.execute('usp_SaveGrade', function (err, recordsets, returnValue) {
            if (err) {
                res.writeHead(200, { "Content-Type": "application/json" });
                res.write(JSON.stringify({ data: "Error Occured: " + err }));
            }
