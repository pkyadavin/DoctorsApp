...
RefreshTokenGrantType.prototype.handle = function(request, client) {
  ...
  return Promise.bind(this)
    ...
    .tap(function(token) {
      return this.revokeToken(token, request);
    })
    .then(function(token) {
      return this.saveToken(token.user, client, token.scope, request);
    });
};
...
RefreshTokenGrantType.prototype.revokeToken = function(token, request) {
  if (this.alwaysIssueNewRefreshToken === false) {
    return Promise.resolve(token);
  }

  return promisify(this.model.revokeToken, 2).call(this.model, token, request)
...
RefreshTokenGrantType.prototype.saveToken = function(user, client, scope, request) {
  var fns = [
