...
AuthorizationCodeGrantType.prototype.handle = function(request, client) {
  ...
  return Promise.bind(this)
    ...
    .tap(function(code) {
      return this.revokeAuthorizationCode(code, request);
    })
    .then(function(code) {
      return this.saveToken(code.user, client, code.authorizationCode, code.scope, request);
    });
};
...
AuthorizationCodeGrantType.prototype.getAuthorizationCode = function(request, client) {
  ...
  return promisify(this.model.getAuthorizationCode, 2).call(this.model, request.body.code, request)
    .then(function(code) {
      if (!code) {
...
AuthorizationCodeGrantType.prototype.revokeAuthorizationCode = function(code, request) {
  return promisify(this.model.revokeAuthorizationCode, 2).call(this.model, code, request)
...
AuthorizationCodeGrantType.prototype.saveToken = function(user, client, authorizationCode, scope, request) {
  var fns = [
    this.validateScope(user, client, scope, request),
    this.generateAccessToken(client, user, scope, request),
    this.generateRefreshToken(client, user, scope, request),
  ...
  return Promise.all(fns)
    ...
    .spread(function(scope, accessToken, refreshToken, accessTokenExpiresAt, refreshTokenExpiresAt) {
      ...
      return promisify(this.model.saveToken, 4).call(this.model, token, client, user, request);
    });
};
